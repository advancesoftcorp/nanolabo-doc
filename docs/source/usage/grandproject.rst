...
.. note::
 ...
 - NanoLaboのライセンスをご購入の際にまとめてNeuralMDのライセンスもご購入いただいた場合、別途ライセンスの設定を行う必要はありません。

 - NeuralMDを後から追加でご購入頂いた場合、ライセンスを更新する操作が必要となります。詳細は、ご利用のOSに対応するインストール手順のページの"ライセンスの更新"（\ :ref:`Windows<licenseupdatew>`\ 、\ :ref:`Linux<licenseupdatel>`\ 、\ :ref:`MacOS<licenseupdatem>`\ ）を参照してください。


.. note::

 リモートの計算サーバー等にジョブ投入を行う場合は、以下の追加設定が必要となります。

 - 計算を実行するリモートのマシンをNeuralMDライセンスのライセンスサーバーとして利用する場合は、計算を実行するリモートのマシンにNeuralMDをインストールし、ライセンスの設定を行ってください(詳細は `NeuralMDのマニュアル <https://neuralmd-doc.readthedocs.io/ja/latest/>`_ を参照してください)。 
 
 - 計算を実行するリモートのマシンとは別のマシンをNeuralMDライセンスのライセンスサーバーとする場合は、計算を実行するリモートのマシン上の$HOME/.hasplm/にhasp_32462.iniファイルを作成し、ライセンスサーバーのIPアドレスを以下の例を参考にして記述してください。
